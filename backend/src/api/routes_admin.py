...
from src.core.pricing_admin import (
    ...
    normalize_pricing_admin_config_for_write,
)
...
def handle_admin_routes(
    ...
    *,
    ...
    if path == "/admin/pricing-config":
        has_access = admin_access or pin_access
        if not has_access:
            return error_response_fn(403, "Admin access required", origin=origin)

        raw = store.get_json(ADMIN_PRICING_CONFIG_KEY)
        config = normalize_pricing_admin_config_for_read(raw)
        ...
        if method == "PUT":
            try:
                normalized = normalize_pricing_admin_config_for_write(parse_json_body(event))
            except ValueError as exc:
                return error_response_fn(400, str(exc), origin=origin)

            normalized["updatedAt"] = now_iso_fn()
            normalized["updatedBy"] = str(claims.get("email") or claims.get("cognito:username") or claims.get("sub") or "unknown")
            store.put_json(ADMIN_PRICING_CONFIG_KEY, normalized)
            return response_fn(
                200,
                {
                    "config": normalized,
